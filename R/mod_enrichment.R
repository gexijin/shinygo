  ...
  tagList(
    ...
    conditionalPanel(
      ...
      p("All query genes are converted to ENSEMBL gene IDs or STRING-db protein IDs,
        ...
        Using these IDs, we manually compile extensive pathway databases from various
        public sources for model organisms."),
      ...
      p("P-values are calculated using the hypergeometric test, and false discovery 
        ...
        enrichment indicates effect size. FDR is bias towards larger pathways 
        due to increased statistical power."),
      ...
      p("We recommend that users provide their own list of background genes, which 
        ...
        is to use all protein-coding genes in the genome. Alternatively, you may select the option
      ...
      in the table and other tabs (Chart, Tree, Network). 
      ...
      p("The Gene Ontology (GO) includes tens of thousands of terms (functional 
        ...
        process is crucial for interpreting GO enrichment results. The top 20 GO terms 
        can be drastically different depending on the sorting method used."),

      tags$ul(
        tags$li("P-value: Reflects the statistical significance of the enrichment.
                Smaller values suggest a lower likelihood of the result occurring by
                chance. FDR q-values adjust P-values for
                multiple testing to control the proportion of type I errors. Large pathways, 
                such as the cell cycle, often show smaller FDRs due to 
            increased statistical power, while smaller pathways might have higher 
            FDRs despite their biological relevance. Enrichment analyses are biased towards larger pathways."),
        tags$li("Fold Enrichment: Measures the magnitude of enrichment. Bigger values
                indicate stronger enrichment and are an important metric of effect size. 
                In addition to FDR, fold enrichment should also be 
      ...
      tags$p("With a default cutoff of FDR < 0.05, thousands of significant GO terms
            ...
            filtering and ranking becomes crucial."),
