#' 04_network UI Function
#'
#' @description Renders the Network tab: an interactive network plot showing
#'   the relationship between significant pathways, controls for changing the
#'   layout/edge cutoff, and download buttons for the plot and its underlying
#'   node/edge tables. A modal dialog (opened by the "Static plot" button)
#'   shows a static version of the same network.
#'
...
#' 04_network Server Functions
#'
#' @param id Internal parameter for {shiny}.
#' @param significant_overlaps A reactive (defined in the main server) that
#'   returns the filtered/sorted/ranked enrichment result list.
...
#'   database input.
#' @param show_pathway_id A reactive returning the value of the "Show pathway
#'   IDs" checkbox input.
#' @param go_button A reactive returning the value of the main "Submit"
#'   action button.
#' @param input_text_b A reactive returning the value of the background genes
#'   textarea; used here only to trigger recalculation when the user changes
...
mod_04_network_server <- function(id, significant_overlaps, select_org, select_go, show_pathway_id, go_button, input_text_b) {
  moduleServer(id, function(input, output, session) {
    # Pathway tables prepared for the interactive and static network plots.
    # The two reactives below are near-duplicates, differing only in which
    # "wrap text" checkbox each reads, so they must be edited together.

    # Data for the interactive network plot
    ...
    # Data for the static network plot (shown in the modal)
    ...
    # Static network plot, shown in the modal opened by the "Static plot"
    # button, and its TIFF download handler.
    ...
    # Interactive visNetwork plot and its download handlers (HTML, plus the
    # underlying node/edge tables as CSV). Each of these rebuilds the same
    # visNetwork graph independently, so they must be edited together.
